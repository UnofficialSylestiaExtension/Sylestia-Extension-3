 ...
 * global object
 */
function create_selected_pets_list() {
...
function mass_release_selected() {
  if (mass_release && lockout_stable === 0) {
    lockout_stable = 1;
    Promise.all(
      selected_pets_list.pet_infos.map((val, index, array) => {
...
function create_release_buttons() {
  if (!document.getElementById("div_top_selection3")) {
    ...
    div_top_3.appendChild(number_selected);
...
// force the patched methods to run in the page environment
var page_script = document.createElement("script");
page_script.textContent = "window.mass_release = false;\n" +
  pet_stall_info.toString() + "\n" +
  create_selected_pets_list.toString() + "\n" +
  "create_selected_pets_list()\n" +
  "stall_select = " + stall_select.toString() + "\n" +
  "stall_hoveron = " + stall_hoveron.toString() + "\n" +
  "stall_hoveroff = " + stall_hoveroff.toString() + "\n" +
  // extension function defs
  toggle_mass_release.toString() + "\n" +
  toggle_mass_release_popup.toString() + "\n" + 
  mass_release_selected.toString();
